...
export async function getRulesetProtectionStatus(branchName: string): Promise<{hasRulesetProtection: boolean; canDelete: boolean; hasPermissionIssues: boolean}> {
  ...
  try {
    ...
    if (rules.data?.some(rule => (rule as {deletion?: boolean}).deletion === false)) {
    ...
    if (httpErr.status === 404) {
      ...
      core.warning(
        `GitHub token lacks permission to read repository rulesets for ${branchName}. Please ensure your token has 'Administration' repository permissions (read). Treating as no ruleset restrictions. Error: ${httpErr.message}`
      )
      hasPermissionIssues = true
    } else {
      // Log other unexpected errors (rate limits, network issues, etc.)
      core.warning(`Failed to check rulesets for ${branchName}: ${httpErr.message ?? 'Unknown error'}. Treating as no ruleset restrictions.`)
    }
  }
