...
| `repository`                    | Repository to scan, in `owner/repo` form. Use this to scan a repository other than the one the workflow runs in (e.g. via a matrix). <br>**⚠️ IMPORTANT: `${{ github.token }}` only has access to the repository the workflow runs in — scanning another repository requires a PAT with access to it.** See the [Token Permissions](#token-permissions) section below.                                                                                                                                                    | Current repository (`GITHUB_REPOSITORY`)                                                                  |
...
#### Scanning Another Repository (`repository`)

The default `${{ github.token }}` is scoped only to the repository the workflow runs in. If you set `repository` to scan a different repository, you must pass a `repo-token` (e.g. a fine-grained or classic PAT) that has been granted access to that target repository, with the same `issues: write` / `contents: write` level permissions described above.
